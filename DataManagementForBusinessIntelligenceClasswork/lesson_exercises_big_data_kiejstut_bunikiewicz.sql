...
LOCATION 's3://lesson-exercises-kiejstut-bunikiewicz/state-names-unpartitioned'
TBLPROPERTIES (
...
LIMIT 10;
--(Run time: 1.1 seconds, Data scanned: 625.93 KB)


-- Count the # of rows in the state_names_unpartitioned table
SELECT COUNT(*)
FROM lesson_exercises.state_names_unpartitioned;
--(Run time: 6.48 seconds, Data scanned: 147.53 MB)
...
WHERE gender = 'F'
GROUP BY year;
--(Run time: 8.81 seconds, Data scanned: 147.53 MB)
...
WHERE gender = 'F'
  AND state = 'CA'
GROUP BY year;
--(Run time: 8.27 seconds, Data scanned: 147.53 MB)
...
FROM lesson_exercises.state_names_unpartitioned;
--(Run time: 10.69 seconds, Data scanned: 147.53 MB)
...
LOCATION 's3://lesson-exercises-kiejstut-bunikiewicz/state-names-partitioned'
TBLPROPERTIES (
  'has_encrypted_date' = 'false'
);
...
FROM lesson_exercises.state_names_partitioned
LIMIT 10;
--(Run time: 1.7 seconds, Data scanned: 2.82 MB)


-- Count the # of rows in the state_names_partitioned table
SELECT COUNT(*)
FROM lesson_exercises.state_names_partitioned;
-- (Run time: 5.54 seconds, Data scanned: 120.6 MB)
...
WHERE gender = 'F'
GROUP BY year;
--(Run time: 3.64 seconds, Data scanned: 67.6 MB)
...
WHERE gender = 'F'
  AND state = 'CA'
GROUP BY year;
--(Run time: 2.31 seconds, Data scanned: 4.36 MB)
...
WITH (
  ...
  external_location = 's3://lesson-exercises-kiejstut-bunikiewicz/state-names-partitioned-parquet-snappy/'
) AS SELECT *
FROM lesson_exercises.state_names_partitioned;
--(Run time: 34.33 seconds, Data scanned: 120.61 MB)
...
LIMIT 10;
--(Run time: 1 seconds, Data scanned: 875.2 KB)


-- Count the # of rows in the state_names_partitioned_parquet_snappy table
SELECT COUNT(*)
FROM lesson_exercises.state_names_partitioned_parquet_snappy;
--(Run time: 3.95 seconds, Data scanned: 0 KB)
...
WHERE gender = 'F'
GROUP BY year;
--(Run time: 2.66 seconds, Data scanned: 47.65 KB)
...
WHERE gender = 'F'
  AND state = 'CA'
GROUP BY year;
--(Run time: 1.15 seconds, Data scanned: 0.9 KB)
...
FROM lesson_exercises.state_names_partitioned_parquet_snappy
WHERE name = 'Jane';
--(Run time: 3.71 seconds, Data scanned: 17.68 MB)
-- 1: 263ms
-- 2: 289ms
-- 3: 255ms
...
WHERE name = 'Angel'
	AND year = 1979;
--(Run time: 4.34 seconds, Data scanned: 11.15 MB)
-- 1: 276ms
-- 2: 248ms
-- 3: 266ms
...
WHERE name = 'Angel'
	AND year = 1979
	AND gender = 'F';
--(Run time: 2.83 seconds, Data scanned: 6.77 MB) 
